...
const KEEP_ALIVE = Symbol(
  process.env.NODE_ENV !== 'production' ? `KeepAlive` : ``
)
const BASE_TRANSITION = Symbol(
  process.env.NODE_ENV !== 'production' ? `BaseTransition` : ``
)
const OPEN_BLOCK = Symbol(
  process.env.NODE_ENV !== 'production' ? `openBlock` : ``
)
const CREATE_BLOCK = Symbol(
  process.env.NODE_ENV !== 'production' ? `createBlock` : ``
)
const CREATE_VNODE = Symbol(
  process.env.NODE_ENV !== 'production' ? `createVNode` : ``
)
...
const RENDER_LIST = Symbol(
  process.env.NODE_ENV !== 'production' ? `renderList` : ``
)
const RENDER_SLOT = Symbol(
  process.env.NODE_ENV !== 'production' ? `renderSlot` : ``
)
const CREATE_SLOTS = Symbol(
  process.env.NODE_ENV !== 'production' ? `createSlots` : ``
)
const TO_DISPLAY_STRING = Symbol(
  process.env.NODE_ENV !== 'production' ? `toDisplayString` : ``
)
const MERGE_PROPS = Symbol(
  process.env.NODE_ENV !== 'production' ? `mergeProps` : ``
)
const TO_HANDLERS = Symbol(
  process.env.NODE_ENV !== 'production' ? `toHandlers` : ``
)
...
const PUSH_SCOPE_ID = Symbol(
  process.env.NODE_ENV !== 'production' ? `pushScopeId` : ``
)
const POP_SCOPE_ID = Symbol(
  process.env.NODE_ENV !== 'production' ? `popScopeId` : ``
)
const WITH_SCOPE_ID = Symbol(
  process.env.NODE_ENV !== 'production' ? `withScopeId` : ``
)
...
var compilerCore = /*@__PURE__*/ build.getAugmentedNamespace(
  compilerCore_esmBundler
)

var shared = /*@__PURE__*/ build.getAugmentedNamespace(shared_esmBundler)

var compilerDom_cjs = build.createCommonjsModule(function (module, exports) {
    ...
    return {
      props: [
        compilerCore.createObjectProperty(
          ...
          exp
            ? compilerCore.createCallExpression(
  ...
  const isEventOptionModifier = /*#__PURE__*/ shared.makeMap(
  ...
  const transformClick = (key, event) => {
    const isStaticClick =
      compilerCore.isStaticExp(key) && key.content.toLowerCase() === 'onclick'
  ...
  const transformOn = (dir, node, context) => {
    return compilerCore.transformOn(dir, node, context, (baseResult) => {
      ...
      if (nonKeyModifiers.length) {
        handlerExp = compilerCore.createCallExpression(
      ...
      if (eventOptionModifiers.length) {
        const modifierPostfix = eventOptionModifiers
          .map(shared.capitalize)
          .join('')
        key = compilerCore.isStaticExp(key)
          ? compilerCore.createSimpleExpression(
  ...
  const stringifyStatic = (children, context, parent) => {
    ...
    const stringifyCurrentChunk = (currentIndex) => {
      if (
        ...
        const staticCall = compilerCore.createCallExpression(
          context.helper(compilerCore.CREATE_STATIC),
          [
            JSON.stringify(
  ...
  function stringifyNode(node, context) {
    ...
    switch (node.type) {
      ...
      case 3 /* COMMENT */:
        return `<!--${shared.escapeHtml(node.content)}-->`
      case 5 /* INTERPOLATION */:
        return shared.escapeHtml(
          shared.toDisplayString(evaluateConstant(node.content))
        )
      case 8 /* COMPOUND_EXPRESSION */:
...
var compilerDom_cjs$1 = /*@__PURE__*/ build.getDefaultExportFromCjs(
  compilerDom_cjs
)

var compilerDom_cjs$2 = /*#__PURE__*/ Object.assign(
  /*#__PURE__*/ Object.create(null),
  compilerDom_cjs,
  {
    default: compilerDom_cjs$1
  }
)

exports.compilerDom_cjs = compilerDom_cjs$2
