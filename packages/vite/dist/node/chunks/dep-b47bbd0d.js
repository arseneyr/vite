...
var build = require('./dep-0a490fab.js')
var os = require('os')
...
function removeWhitespace(value, format) {
  ...
  var spaceAroundRelation =
    format && format.spaces[Spaces$1.AroundSelectorRelation]
  var i, l
  ...
  for (i = 0, l = value.length; i < l; i++) {
    ...
    isNewLineWin =
      character == marker.NEW_LINE_NIX && value[i - 1] == marker.CARRIAGE_RETURN
    isQuoted = isSingleQuoted || isDoubleQuoted
    isRelation =
      !isAttribute &&
      !isEscaped &&
      roundBracketLevel === 0 &&
      RELATION_PATTERN$1.test(character)
...
function beforeSlash(token, valueIndex) {
  return (
    token[valueIndex + 1] && token[valueIndex + 1][1] == marker.FORWARD_SLASH
  )
}
...
function property$1(context, tokens, position, lastPropertyAt) {
  ...
  var isPropertyBlock =
    propertyValue && propertyValue[0] === token.PROPERTY_BLOCK
...
function value$1(context, token$1) {
  ...
  if (token$1[2][0] == token.PROPERTY_BLOCK) {
    ...
    for (j = 2, m = token$1.length; j < m; j++) {
      ...
      if (
        j < m - 1 &&
        (inFilter(token$1) || !disallowsSpace(context, token$1, j))
      ) {
...
function openBrace(context, where, needsPrefixSpace) {
  if (context.format) {
    ...
    return (
      (needsPrefixSpace && allowsSpace(context, Spaces.BeforeBlockBegins)
        ? marker.SPACE
        : emptyCharacter) +
    ...
    if (!PROPERTY_NAME_PATTERN.test(name)) {
      ...
      context.warnings.push(
        "Invalid property name '" +
          name +
          "' at " +
          formatPosition_1(propertyToken[1][2][0]) +
          '. Ignoring.'
      )
      ...
      if (type == token.PROPERTY_BLOCK) {
        ...
        context.warnings.push(
          'Invalid value token at ' +
            formatPosition_1(value[0][1][2][0]) +
            '. Ignoring.'
        )
        break
      }
      ...
      if (valueIsUrl && !context.validator.isUrl(value)) {
        ...
        context.warnings.push(
          "Broken URL '" +
            value +
            "' at " +
            formatPosition_1(property.value[j][2][0]) +
            '. Ignoring.'
        )
        break
      }
...
function optimizeComment(token, options) {
  if (
    ...
    (options.level[OptimizationLevel$5.One].specialComments == 'all' ||
      options.commentsKept <
        options.level[OptimizationLevel$5.One].specialComments)
  ) {
    options.commentsKept++
    return
  }
...
function cleanupCharsets(tokens) {
  ...
  for (var i = 0, l = tokens.length; i < l; i++) {
    ...
    if (token$1[0] != token.AT_RULE) continue

    if (!CHARSET_REGEXP.test(token$1[1])) continue
    ...
    if (hasCharset || token$1[1].indexOf(CHARSET_TOKEN) == -1) {
      ...
      tokens.unshift([
        token.AT_RULE,
        token$1[1].replace(CHARSET_REGEXP, CHARSET_TOKEN)
      ])
    }
  }
}
  ...
  for (var i = 0, l = tokens.length; i < l; i++) {
    ...
    switch (token$1[0]) {
      case token.AT_RULE:
        ...
        token$1[1] = levelOptions.tidyAtRules
          ? tidyAtRule_1(token$1[1])
          : token$1[1]
        mayHaveCharset = true
        break
      case token.AT_RULE_BLOCK:
        ...
        break
      case token.NESTED_BLOCK:
        token$1[1] = levelOptions.tidyBlockScopes
          ? tidyBlock_1(token$1[1], spaceAfterClosingBrace)
          : token$1[1]
      ...
      case token.COMMENT:
        ...
        break
      case token.RULE:
        token$1[1] = levelOptions.tidySelectors
          ? tidyRules_1(
              token$1[1],
              !ie7Hack,
              adjacentSpace,
        ...
        token$1[1] =
          token$1[1].length > 1
            ? sortSelectors_1(token$1[1], levelOptions.selectorsSortingMethod)
            : token$1[1]
        optimizeBody(token$1[1], token$1[2], context)
        afterRules = true
        break
    ...
    if (
      (token$1[0] == token.COMMENT && token$1[1].length === 0) ||
      (levelOptions.removeEmpty &&
        (token$1[1].length === 0 || (token$1[2] && token$1[2].length === 0)))
    ) {
      tokens.splice(i, 1)
      i--
      l--
  ...
  if (!values[index]) {
    throw new invalidPropertyError(
      'Missing font values at ' +
        formatPosition_1(property.all[property.position][1][2][0]) +
        '. Ignoring.'
    )
  }
...
function widthStyleColor(property, compactable, validator) {
  ...
  if (values.length > 0) {
    ...
    match =
      matches.length > 1 && (matches[0][1] == 'none' || matches[0][1] == 'auto')
        ? matches[1]
        : matches[0]
    if (match) {
      width.value = [match]
...
function animationIterationCount(validator, value1, value2) {
  if (
    !understandable_1(validator, value1, value2, 0, true) &&
    !(
...
function cloneDescriptor(propertyName, prefix) {
  ...
  if ('componentOf' in clonedDescriptor) {
    clonedDescriptor.componentOf = clonedDescriptor.componentOf.map(function (
      shorthandName
    ) {
  ...
  if ('components' in clonedDescriptor) {
    clonedDescriptor.components = clonedDescriptor.components.map(function (
      longhandName
    ) {
      ...
      try {
        ...
        if (descriptor.shorthandComponents) {
          for (j = 0, m = property.components.length; j < m; j++) {
            ...
            component.components = compactable_1[component.name].breakUp(
              component,
              compactable_1,
              validator
            )
...
function mergeable(components) {
  ...
  for (componentName in components) {
    ...
    restoreFromOptimizing_1(
      [component.all[component.position]],
      restoreWithComponents_1
    )
...
function replaceWithInheritBestFit(
  ...
  var isLonghandsShorter =
    serializeBody$5(longhandTokensSequence).length <
    serializeBody$5(shorthandTokensSequence).length
...
function isComponentOf(property1, property2, shallow) {
  return (
    ...
    (!shallow &&
      !!compactable_1[property1.name].shorthandComponents &&
...
function overridesNonComponentShorthand(property1, property2) {
  return (
    property1.name in compactable_1 &&
    'overridesShorthands' in compactable_1[property1.name] &&
    compactable_1[property1.name].overridesShorthands.indexOf(property2.name) >
      -1
  )
}
...
function overrideProperties(properties, withMerging, compatibility, validator) {
  ...
  propertyLoop: for (i = properties.length - 1; i >= 0; i--) {
    ...
    traverseLoop: for (j = i - 1; j >= 0; j--) {
      ...
      if (hasInherit_1(right)) continue
      ...
      if (right.shorthand && isComponentOf_1(right, left)) {
        ...
        if (
          everyValuesPair_1(mayOverride.bind(null, validator), left, component)
        ) {
          left.unused = true
        }
      } else if (
        right.shorthand &&
        overridesNonComponentShorthand_1(right, left)
      ) {
        ...
        left.shorthand &&
        !right.shorthand &&
        isComponentOf_1(left, right, true)
      ) {
        ...
        if (!isMergeableShorthand_1(left)) continue

        component = findComponentIn_1(left, right)
        if (
          everyValuesPair_1(mayOverride.bind(null, validator), component, right)
        ) {
          ...
          var nonMergeableValue =
            compactable_1[right.name].nonMergeableValue === right.value[0][1]
          ...
          if (
            ...
            (hasInherit_1(left) || hasInherit_1(right))
          )
            continue
        ...
        var rightRestored = compactable_1[right.name].restore(
          right,
          compactable_1
        )
...
function optimizeProperties(properties, withOverriding, withMerging, context) {
  ...
  var _properties = wrapForOptimizing$1(
    properties,
    false,
...
function mergeAdjacent(tokens, context) {
  ...
  var selectorsSortingMethod =
    options.level[OptimizationLevel$3.One].selectorsSortingMethod
  ...
  for (var i = 0, l = tokens.length; i < l; i++) {
    ...
    if (
      ...
      serializeRules$8(token$1[1]) == serializeRules$8(lastToken[1])
    ) {
      ...
      lastToken[0] == token.RULE &&
      serializeBody$4(token$1[2]) == serializeBody$4(lastToken[2]) &&
      isMergeable_1(
        serializeRules$8(token$1[1]),
        mergeablePseudoClasses,
        mergeablePseudoElements,
        multiplePseudoMerging
      ) &&
      ...
      lastToken[1] = tidyRules_1(
        lastToken[1].concat(token$1[1]),
        false,
        adjacentSpace,
        false,
        context.warnings
      ...
      lastToken[1] =
        lastToken.length > 1
          ? sortSelectors_1(lastToken[1], selectorsSortingMethod)
          : lastToken[1]
      token$1[2] = []
    } else {
      lastToken = token$1
...
function canReorderSingle$2(left, right, cache) {
  ...
  if (
    ...
    !rulesOverlap_1(rightSelector, leftSelector, false)
  )
    return true
  if (!specificitiesOverlap_1(leftSelector, rightSelector, cache)) return true
...
function mergeMediaQueries(tokens, context) {
  var mergeSemantically =
    context.options.level[OptimizationLevel$2.Two].mergeSemantically
...
function allSameRulePropertiesCanBeReordered(
  ...
  for (i = 0, l = movedProperties.length; i < l; i++) {
    ...
    for (j = 0, m = traversedProperties.length; j < m; j++) {
      ...
      if (
        rulesOverlap_1(movedRule, traversedRule, true) &&
...
function mergeNonAdjacentByBody(tokens, context) {
  var options = context.options
  var mergeSemantically =
    options.level[OptimizationLevel$1.Two].mergeSemantically
  var adjacentSpace = options.compatibility.selectors.adjacentSpace
  var selectorsSortingMethod =
    options.level[OptimizationLevel$1.One].selectorsSortingMethod
  var mergeablePseudoClasses =
    options.compatibility.selectors.mergeablePseudoClasses
  var mergeablePseudoElements =
    options.compatibility.selectors.mergeablePseudoElements
  ...
  for (var i = tokens.length - 1; i >= 0; i--) {
    var token$1 = tokens[i]
    if (token$1[0] != token.RULE) continue

    if (
      token$1[2].length > 0 &&
      !mergeSemantically &&
      unsafeSelector(serializeRules$5(token$1[1]))
    )
      candidates = {}
    ...
    if (
      ...
      if (token$1[2].length > 0) {
        token$1[1] = tidyRules_1(
          oldToken[1].concat(token$1[1]),
          false,
          adjacentSpace,
          false,
          context.warnings
        ...
        token$1[1] =
          token$1[1].length > 1
            ? sortSelectors_1(token$1[1], selectorsSortingMethod)
            : token$1[1]
      } else {
...
function reduceComplexNonAdjacentCases(tokens, candidates, options, context) {
  ...
  allSelectors: for (var complexSelector in candidates) {
    ...
    for (var j = 0, m = selectors.length; j < m; j++) {
      ...
      if (
        serializeBody$2(reducedBodies[reducedBodies.length - 1]) !=
        serializeBody$2(reducedBodies[0])
      )
...
function removeDuplicateFontAtRules(tokens) {
  ...
  for (i = 0, l = tokens.length; i < l; i++) {
    ...
    if (
      token$1[0] != token.AT_RULE_BLOCK &&
      token$1[1][0][1] != FONT_FACE_SCOPE
    ) {
      continue
    }
...
function removeDuplicates(tokens) {
  ...
  for (i = 0, l = moreThanOnce.length; i < l; i++) {
    ...
    for (var j = matched[id].length - 1; j >= 0; j--) {
      ...
      if (bodies.indexOf(body) > -1) token$1[2] = []
...
function matchCounterStyle(token$1, atRules) {
  ...
  if (
    token$1[0] == token.AT_RULE_BLOCK &&
    token$1[1][0][1].indexOf('@counter-style') === 0
  ) {
...
function matchKeyframe(token$1, atRules) {
  ...
  if (
    token$1[0] == token.NESTED_BLOCK &&
    keyframeRegex.test(token$1[1][0][1])
  ) {
    match = token$1[1][0][1].split(' ')[1]
    atRules[match] = atRules[match] || []
    atRules[match].push(token$1)
...
function loadRemoteResource(uri, inlineRequest, inlineTimeout, callback) {
  ...
  requestOptions = override_1(url__default.parse(uri), inlineRequest || {})
  ...
  fetch =
    (proxyProtocol && !isHttpsResource_1(proxyProtocol)) ||
    isHttpResource_1(uri)
      ? http__default.get
      : https__default.get
...
function isAllowedResource(uri, isRemote, rules) {
  ...
  match = isRemote ? url__default.parse(uri).host : uri

  absoluteUri = isRemote ? uri : path__default.resolve(uri)
  ...
  for (i = 0; i < rules.length; i++) {
    ...
    if (isNegated && isRemote && isRemoteRule(normalizedRule)) {
      ...
      !isRemote &&
      absoluteUri.indexOf(path__default.resolve(rule)) === 0
    ) {
      allowed = true
...
function isRemoteRule(rule) {
  return (
    isRemoteResource_1(rule) ||
    url__default.parse(HTTP_PROTOCOL + '//' + rule).host == rule
  )
}
...
function rebaseLocalMap(sourceMap, sourceUri, rebaseTo) {
  ...
  sourceMap.sources = sourceMap.sources.map(function (source) {
    return path__default.relative(
      rebaseTo,
      path__default.resolve(absoluteUriDirectory, source)
    )
  })
...
function extractInputSourceMapFromDataUri(uri) {
  ...
  var data =
    encoding == 'utf8'
      ? build.commonjsGlobal.unescape(dataUriMatch[4])
      : dataUriMatch[4]
...
function loadOriginalSourceFromLocalUri(relativeUri, loadContext) {
  ...
  if (
    !fs__default.existsSync(absoluteUri) ||
    !fs__default.statSync(absoluteUri).isFile()
  ) {
    loadContext.warnings.push(
...
function relative(uri, rebaseConfig) {
  return path__default.relative(
    rebaseConfig.toBase,
    path__default.join(rebaseConfig.fromBase || '', uri)
  )
}
...
function rebaseSourceMapComment(token, rebaseConfig) {
  ...
  if (matches && matches[1].indexOf('data:') === -1) {
    token[1] = token[1].replace(
      matches[1],
      rewriteUrl_1(matches[1], rebaseConfig, true)
    )
  }
}
  ...
  for (; position.index < source.length; position.index++) {
    ...
    if (isEscaped) {
      ...
      externalContext.warnings.push(
        "Unexpected '*/' at " +
          formatPosition_1([position.line, position.column, position.source]) +
          '.'
      )
      buffer = []
    } else if (character == marker.SINGLE_QUOTE && !isQuoted) {
    ...
    position.line =
      isNewLineWin || isNewLineNix || isCarriageReturn
        ...
        : position.line
    position.column =
      isNewLineWin || isNewLineNix || isCarriageReturn ? 0 : position.column + 1
  }
...
function normalizeUri(uri) {
  ...
  absoluteUri = path__default.isAbsolute(uri) ? uri : path__default.resolve(uri)
...
function fromStyles(styles, context, parentInlinerContext, callback) {
  ...
  if (!context.source) {
    ...
    rebaseConfig.fromBase = path__default.dirname(
      path__default.resolve(context.source)
    )
    rebaseConfig.toBase = context.options.rebaseTo
  }
  ...
  function minify(input, options, maybeSourceMap, maybeCallback) {
    ...
    return runner(context.localOnly)(function () {
      return readSources_1(input, context, function (tokens) {
        var serialize = context.options.sourceMap ? sourceMaps : simple
...
var index = /*#__PURE__*/ Object.assign(
