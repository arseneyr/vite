...
var build = require('./chunks/dep-0a490fab.js')
var os = require('os')
...
var zlib_1__default = /*#__PURE__*/ _interopDefaultLegacy(zlib_1)
...
var mimeTypes = build.createCommonjsModule(function (module, exports) {
  /**
   * Module dependencies.
   * @private
   */
...
var node = build.createCommonjsModule(function (module, exports) {
  /**
  ...
  if (1 !== fd && 2 !== fd) {
    util__default.deprecate(function () {},
  ...
  exports.formatters.O = function (v) {
    this.inspectOpts.colors = this.useColors
    return util__default.inspect(v, this.inspectOpts)
  }
  ...
  function log() {
    return stream.write(
      util__default.format.apply(util__default, arguments) + '\n'
    )
  }
  ...
  function createWritableStdioStream(fd) {
    ...
    switch (tty_wrap.guessHandleType(fd)) {
      case 'TTY':
        stream = new tty__default.WriteStream(fd)
        stream._type = 'tty'
...
var src = build.createCommonjsModule(function (module) {
...
function compression(options) {
  ...
  return function compression(req, res, next) {
    ...
    onHeaders_1(res, function onResponseHeaders() {
      ...
      // vary
      build.vary_1(res, 'Accept-Encoding')
  ...
  httpServer.listen(port, () => {
    ...
    Object.keys(interfaces).forEach((key) =>
      (interfaces[key] || [])
        ...
        .forEach(({ type, host }) => {
          const url = `${protocol}://${host}:${build.source.bold(port)}${base}`
          logger.info(`  > ${type} ${build.source.cyan(url)}`)
        })
    )
    if (options.open) {
...
cli
  ...
  .action(async (root, options) => {
    ...
    // is ok here
    const { createServer } = await Promise.resolve()
      .then(function () {
        return require('./chunks/dep-0a490fab.js')
      })
      .then(function (n) {
        return n.index$1
      })
    try {
      ...
      build
        ...
        .error(build.source.red(`error when starting dev server:\n${e.stack}`))
      process.exit(1)
    }
  })
// build
cli
  ...
  .option('-m, --mode <mode>', `[string] set env mode`)
  .action(async (root, options) => {
    const { build: build$1 } = await Promise.resolve()
      .then(function () {
        return require('./chunks/dep-0a490fab.js')
      })
      .then(function (n) {
    ...
    try {
      ...
      build
        .createLogger(options.logLevel)
        .error(build.source.red(`error during build:\n${e.stack}`))
      process.exit(1)
    }
  })
// optimize
cli
  ...
  .action(async (root, options) => {
    const { optimizeDeps } = await Promise.resolve()
      .then(function () {
        return require('./chunks/dep-0a490fab.js')
      })
      .then(function (n) {
    ...
    try {
      ...
      build
        .createLogger(options.logLevel)
        .error(build.source.red(`error when optimizing deps:\n${e.stack}`))
      process.exit(1)
    }
  })
cli
  ...
  .action(async (root, options) => {
    try {
      ...
      build
        .createLogger(options.logLevel)
        .error(
          build.source.red(`error when starting preview server:\n${e.stack}`)
        )
      process.exit(1)
    }
  })
